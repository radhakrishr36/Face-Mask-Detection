...
def detect_and_predict_mask(frame, faceNet, maskNet):
	...
	for i in range(0, detections.shape[2]):
		...
		if confidence > args["confidence"]:
			...
			print("original pixel size::",face.shape)
...
while True:
	...
	for (box, pred,face) in zip(locs, preds,faces):
		...
		if len(faces)>0 and label_orginal=='Mask' :
			
			img = Image.fromarray(cv2.rectangle(frame, (startX, startY), (endX, endY), color, 2), 'RGB')
			img.save(file_path+filename_mask)

		elif len(faces)>0 and label_orginal=='No Mask' :
